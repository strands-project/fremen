...
#include "froctomap/Update.h"
#include "froctomap/SaveLoad.h"
#include "froctomap/Evaluate.h"
#include "froctomap/Recover.h"
#include "froctomap/Estimate.h"
...
#define LIM_MAX_X 0.050
#define LIM_MAX_Y 0.050
#define LIM_MAX_Z 4.70
#define MAX_GRIDS 10
...
int numMaps = 0;
int poseCount = 0;
CFremenGrid *gridArray[MAX_GRIDS];
CFremenGrid *gridPtr;
...
bool load_octomap(froctomap::SaveLoad::Request  &req, froctomap::SaveLoad::Response &res)
{
	currentMap = -1;
	for (int i =0;i<numMaps;i++)
	{
...
bool evaluate_octomap(froctomap::Evaluate::Request  &req, froctomap::Evaluate::Response &res)
{
	currentMap = -1;
	for (int i =0;i<numMaps;i++){
		if (strcmp(req.mapname.c_str(),gridArray[i]->name)==0) currentMap = i; 
	...
	{
		ROS_INFO("Error rate order (all/nonempty/dynamic) is %i %.5f/%.5f/%.5f.",i,e[i].all,e[i].nonempty,e[i].dynamic);
...
bool save_octomap(froctomap::SaveLoad::Request  &req, froctomap::SaveLoad::Response &res)
{
	currentMap = -1;
	for (int i =0;i<numMaps;i++){
		if (strcmp(req.mapname.c_str(),gridArray[i]->name)==0) currentMap = i; 
...
bool update_octomap(froctomap::Update::Request  &req, froctomap::Update::Response &res)
{
	currentMap = -1;
	for (int i =0;i<numMaps;i++){
		if (strcmp(req.mapname.c_str(),gridArray[i]->name)==0) currentMap = i; 
...
bool recover_octomap(froctomap::Recover::Request  &req, froctomap::Recover::Response &res)
{
	currentMap = -1;
	for (int i =0;i<numMaps;i++){
		if (strcmp(req.mapname.c_str(),gridArray[i]->name)==0) currentMap = i; 
...
bool estimate_octomap(froctomap::Estimate::Request  &req, froctomap::Estimate::Response &res)
{
	currentMap = -1;
	for (int i =0;i<numMaps;i++){
		if (strcmp(req.mapname.c_str(),gridArray[i]->name)==0) currentMap = i; 
...
int main(int argc,char *argv[])
{
	...
	n.param<std::string>("filename", loadfilename, "");
...
//	ros::Subscriber sub_name = n.subscribe ("/current_node", 1, name_cb);
	...
	//Publishers:
	ros::Publisher octomap_pub = n.advertise<octomap_msgs::Octomap>("/FrOctomap/octomap", 100);
	octomap_pub_ptr = &octomap_pub;

	ros::Publisher retrieve_pub = n.advertise<visualization_msgs::MarkerArray>("/FrOctomap/recovered", 100);
	retrieve_pub_ptr = &retrieve_pub;

	ros::Publisher estimate_pub = n.advertise<visualization_msgs::MarkerArray>("/FrOctomap/estimated", 100);
	estimate_pub_ptr = &estimate_pub;
	
	ros::Publisher clock_pub = n.advertise<visualization_msgs::Marker>("/FrOctomap/clock", 100);
	clock_pub_ptr = &clock_pub;

	//Services:
	ros::ServiceServer retrieve_service = n.advertiseService("/FrOctomap/recover", recover_octomap);
	ros::ServiceServer estimate_service = n.advertiseService("/FrOctomap/estimate", estimate_octomap);
	ros::ServiceServer save_service = n.advertiseService("/FrOctomap/save", save_octomap);
	ros::ServiceServer load_service = n.advertiseService("/FrOctomap/load", load_octomap);
	ros::ServiceServer evaluate_service = n.advertiseService("/FrOctomap/evaluate", evaluate_octomap);
	ros::ServiceServer update_service = n.advertiseService("/FrOctomap/update", update_octomap);
