...
class CFrelementSet
{
	public:
		...
		bool add(const char *name,uint32_t times[],unsigned char states[],int length);
		...
		bool estimate(const char *name,uint32_t times[],float probs[],int length,int order);

		/*estimate entropies of the given state for the given times - the entropy array is an output
		  returns false if the state with the given ID is not present in the collection
		  otherwise returns true and fills the probs array with the calculated predictions*/
		bool estimateEntropy(const char *name,uint32_t times[],float entropy[],int length,int order);
