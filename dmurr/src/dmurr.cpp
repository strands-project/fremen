...
#include <tf/tf.h>
//MIGRATE #include <mongodb_store/message_store.h>
...
#include <dmurr/TopologicalPredictionAction.h>

const mongo::BSONObj EMPTY_BSON_OBJ;

//MIGRATE using namespace mongodb_store;
...
int numEdges = 0;
CEdgeStatistics *edgeArray[1000];
char mapName[1000];
typedef actionlib::SimpleActionServer<dmurr::TopologicalPredictionAction> Server;
Server* server;
dmurr::TopologicalPredictionResult result;
dmurr::TopologicalPredictionFeedback feedback;
...
void actionServerCallback(const dmurr::TopologicalPredictionGoalConstPtr& goal, Server* as)
{
	char message[1000];
...
{
	ros::init(argc, argv, "dmurr");
	n = new ros::NodeHandle();
	server = new Server(*n, "topologicalPrediction", boost::bind(&actionServerCallback, _1, server), false);
	server->start();
	//retrieveData();
	//loadData(argv[1]);
	//printEdges(argv[2]);
