...
{
	fread(name,1,100,file);
	fread(&firstTime,sizeof(long int),1,file);
	fread(&length,sizeof(int),1,file);
	fread(&durations,sizeof(float),length,file);
	fread(&results,sizeof(unsigned char),length,file);
	fread(&times,sizeof(int),length,file);
}
